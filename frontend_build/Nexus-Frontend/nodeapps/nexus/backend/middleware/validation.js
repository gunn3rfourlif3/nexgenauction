...
// Canonical lists for case-insensitive validation
const allowedCategories = ['Art', 'Collectibles', 'Vehicles', 'Antiques', 'Jewelry', 'Books & Manuscripts'];
const allowedStatuses = ['upcoming', 'active', 'paused', 'ended', 'cancelled'];

// Helper to normalize category/status to canonical case
const toCanonical = (value, allowed) => {
  if (typeof value !== 'string') return value;
  const idx = allowed.map(v => v.toLowerCase()).indexOf(value.toLowerCase());
  return idx >= 0 ? allowed[idx] : value;
};

// Helper to sanitize price strings (remove commas/currency signs)
...
const validateAuctionQuery = [
  // Category: accept case-insensitive match and sanitize to canonical value
  query('category')
    .optional()
    .customSanitizer((val) => toCanonical(val, allowedCategories))
    .custom((val) => typeof val === 'string' && allowedCategories.map(v => v.toLowerCase()).includes(val.toLowerCase()))
    .withMessage('Invalid category'),

  // Status: accept case-insensitive match and sanitize to canonical value
  query('status')
    .optional()
    .customSanitizer((val) => toCanonical(val, allowedStatuses))
    .custom((val) => typeof val === 'string' && allowedStatuses.map(v => v.toLowerCase()).includes(val.toLowerCase()))
    .withMessage('Invalid status'),
  
  // Prices: sanitize inputs like "1,000" or "$250" and validate
  query('minPrice')
    .optional()
    .customSanitizer(sanitizePrice)
    .isFloat({ min: 0 })
