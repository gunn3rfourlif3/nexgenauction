...
class InvoiceService {
  constructor() {
    ...
    // Default VAT rate fallback (per-lot overrides take precedence)
    this.defaultVatRate = 0.15;
  ...
  calculateVat(amount, vatRate) {
    return Math.round(amount * (vatRate || this.defaultVatRate) * 100) / 100;
  }
  ...
  async generateInvoiceData(paymentId, options = {}) {
    try {
      ...
      // Per-lot fee overrides
      const commissionApplicable = (auction.commissionApplicable !== undefined) ? Boolean(auction.commissionApplicable) : true;
      const buyersCommissionRate = (typeof auction.buyersCommissionRate === 'number') ? auction.buyersCommissionRate : undefined;

      const vatApplicable = Boolean(auction.vatApplicable);
      const vatRate = (typeof auction.vatRate === 'number') ? auction.vatRate : this.defaultVatRate;

      const stcApplicable = Boolean(auction.stcApplicable);
      const stcRate = (typeof auction.stcRate === 'number') ? auction.stcRate : 0;

      // Commission calculation
      const buyersPremium = commissionApplicable
        ? (typeof buyersCommissionRate === 'number' ? Math.round(winningBidAmount * buyersCommissionRate * 100) / 100 : this.calculateBuyersPremium(winningBidAmount))
        : 0;
      ...
      // STC calculation (on bid)
      const stcAmount = stcApplicable && stcRate > 0 ? Math.round(winningBidAmount * stcRate * 100) / 100 : 0;

      // VAT calculation
      const vatOnCommission = buyersPremium > 0 ? this.calculateVat(buyersPremium, vatRate) : 0;
      const vatOnBid = vatApplicable ? this.calculateVat(winningBidAmount, vatRate) : 0;
      const vatAmount = vatOnCommission + vatOnBid;

      // Subtotal (excluding VAT)
      const subtotal = winningBidAmount + buyersPremium + stcAmount + shippingCost;
      ...
      const invoiceData = {
        ...
        lineItems: [
          ...
          ...(buyersPremium > 0 ? [{
            description: "Buyer’s Commission",
            quantity: 1,
            unitPrice: buyersPremium,
            total: buyersPremium,
            note: (typeof buyersCommissionRate === 'number')
              ? `Commission rate ${(buyersCommissionRate * 100).toFixed(2)}% applied`
              : `Sliding scale: 25% first $1,000, 20% next $9,000, 15% above $10,000`
          }] : []),
          ...(vatOnBid > 0 ? [{
            description: `VAT on Bid (${(vatRate * 100).toFixed(2)}%)`,
            quantity: 1,
            unitPrice: vatOnBid,
            total: vatOnBid
          }] : []),
          ...(vatOnCommission > 0 ? [{
            description: `VAT on Commission (${(vatRate * 100).toFixed(2)}%)`,
            quantity: 1,
            unitPrice: vatOnCommission,
            total: vatOnCommission
          }] : []),
          ...(stcAmount > 0 ? [{
            description: `STC (${(stcRate * 100).toFixed(2)}%)`,
            quantity: 1,
            unitPrice: stcAmount,
            total: stcAmount
          }] : []),
  ...
  async generatePDFInvoice(invoiceData) {
    try {
      ...
      return new Promise((resolve, reject) => {
        ...
        yPosition += 15;
        doc.text(`VAT (${(invoiceData.amounts.vatRate * 100).toFixed(2)}%):`, 350, yPosition)
          .text(`${invoiceData.currency} ${invoiceData.amounts.vat.toFixed(2)}`, 450, yPosition);
