...
const getAuctions = async (req, res) => {
  try {
    ...
    // On failure, optionally fall back to dev mock catalog when enabled
    try {
      const allowFailureFallback = (process.env.ENABLE_DEV_MOCK_ON_FAILURE === 'true');
      const devModeFallback = (process.env.ENABLE_DEV_MOCK === 'true') ||
        (process.env.NODE_ENV === 'development' && process.env.FORCE_DB_CONNECTION !== 'true');
      if (allowFailureFallback || devModeFallback) {
        const {
          page = 1,
          limit = 12,
          category,
      ...
      console.error('Dev fallback after failure error:', fallbackErr);
    }
...
const updateAuction = async (req, res) => {
  try {
    ...
    if (Array.isArray(updates.tags)) {
      updates.tags = updates.tags.map(t => String(t).toLowerCase().trim()).filter(Boolean);
    }
  ...
  const auction = await Auction.findById(id);
    ...
    // Broadcast auction update to live clients (e.g., time changes)
    try {
      ...
      if (io && io.to) {
        io.to(`auction-${id}`).emit('auction-update', {
          ...
          bidCount: Array.isArray(auction.bids) ? auction.bids.length : undefined
        });
      }
    } catch (_) {}
...
const placeBid = async (req, res) => {
  ...
  if (auction.seller.toString() === req.user._id.toString()) {
    ...
    // Enforce deposit requirement at auction level
    if (auction.depositRequired) {
      const AuctionRegistration = require('../models/AuctionRegistration');
      const reg = await AuctionRegistration.findOne({ auctionId: id, userId: req.user._id });
      const ok = reg && (reg.status === 'approved' || reg.status === 'deposit_received');
      if (!ok) {
        return res.status(403).json({ success: false, message: 'Deposit required to bid on this auction' });
      }
    }
