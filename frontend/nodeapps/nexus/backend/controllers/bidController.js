...
console.info('[BidController] Loaded with dev fallback guards');
const getIncrementForAuction = (auction, currentAmount) => {
  const inc = auction && typeof auction.bidIncrement === 'number' && auction.bidIncrement > 0 ? auction.bidIncrement : null;
  return inc || calculateMinimumIncrement(currentAmount);
};
...
const calculateMinimumIncrement = (currentBid) => {
  ...
  const placeBid = async (req, res) => {
    ...
    // Ensure bidder is approved to bid on this auction
    const AuctionRegistration = require('../models/AuctionRegistration');
    if (!usedDevMock) {
      const reg = await AuctionRegistration.findOne({ auctionId: auctionId, userId });
      const allowed = reg && (reg.status === 'approved' || reg.status === 'deposit_received');
      if (!allowed) {
        return res.status(400).json({ success: false, message: 'You must be approved to bid on this auction' });
      }
    }

    // Ensure account completeness before bidding
    let user = null;
    const requireProfileComplete = (process.env.REQUIRE_PROFILE_COMPLETE || 'true') !== 'false';
    if (!usedDevMock) {
      user = await User.findById(userId).select('accountBalance currency phone address isVerified');
      if (!user) {
      ...
      if (requireEmailVerified && !user.isVerified) {
        return res.status(400).json({ success: false, message: 'Please verify your email before bidding' });
      }
      if (requireProfileComplete) {
        ...
        if (!hasPhone || !hasAddress) {
          return res.status(400).json({ success: false, message: 'Please complete your account (phone and address) before bidding' });
        }
      }
    ...
    const minimumIncrement = getIncrementForAuction(auction, currentAmount);
    ...
    res.status(201).json({
      ...
      data: {
        bid: newBid,
        minimumNextBid: amount + getIncrementForAuction(auction, amount)
      }
    });
...
const processAutoBids = async (auctionId, newBidAmount, excludeUserId, io) => {
  try {
    ...
    for (const autoBid of autoBids) {
      ...
      const minimumIncrement = getIncrementForAuction(auction, newBidAmount);
...
const getBidHistory = async (req, res) => {
  try {
    ...
    console.info('[getBidHistory] devFallbackEnabled=', devFallbackEnabled, 'isDbConnected=', isDbConnected, 'env=', process.env.NODE_ENV, 'FORCE_DB_CONNECTION=', process.env.FORCE_DB_CONNECTION, 'ENABLE_DEV_MOCK=', process.env.ENABLE_DEV_MOCK);
...
const getCurrentBid = async (req, res) => {
  try {
    ...
    const devFallbackEnabled = (process.env.ENABLE_DEV_MOCK === 'true') ||
      (!isDbConnected) ||
      (((process.env.NODE_ENV || 'development') !== 'production') && process.env.FORCE_DB_CONNECTION !== 'true');
    console.info('[getCurrentBid] devFallbackEnabled=', devFallbackEnabled, 'isDbConnected=', isDbConnected, 'env=', process.env.NODE_ENV, 'FORCE_DB_CONNECTION=', process.env.FORCE_DB_CONNECTION, 'ENABLE_DEV_MOCK=', process.env.ENABLE_DEV_MOCK);

    if (devFallbackEnabled) {
      const auction = devMockStore.getAuction(auctionId);
      const all = Array.isArray(auction.bids) ? auction.bids.slice() : [];
      const highestBid = all.length > 0
        ? all.reduce((prev, cur) => ((prev && prev.amount > cur.amount) ? prev : cur))
        : null;

      const currentPrice = typeof auction.currentBid === 'number' ? auction.currentBid : auction.startingPrice;
      const minimumNextBid = currentPrice + getIncrementForAuction(auction, currentPrice);
      const totalBids = all.length;
      ...
      return res.json({
        ...
        data: {
          ...
          minimumNextBid,
          totalBids,
          minimumIncrement: getIncrementForAuction(auction, currentPrice)
        }
      });
    }
    ...
    if (!auction) {
      ...
      const minimumNextBid = currentPrice + getIncrementForAuction(auction, currentPrice);
    const totalBids = devFallbackEnabled && auction && Array.isArray(auction.bids)
      ? auction.bids.length
      : await Bid.countDocuments({ auction: auctionId, isActive: true });
...
const setAutoBid = async (req, res) => {
  try {
    ...
    // Enforce approved registration before allowing auto-bid setup
    const AuctionRegistration = require('../models/AuctionRegistration');
    const reg = await AuctionRegistration.findOne({ auctionId, userId });
    if (!reg || (reg.status !== 'approved' && reg.status !== 'deposit_received')) {
      return res.status(400).json({ success: false, message: 'You must be approved to bid on this auction' });
    }
    ...
    const user = await User.findById(userId).select('accountBalance phone address');
    const requireProfileComplete = (process.env.REQUIRE_PROFILE_COMPLETE || 'true') !== 'false';
    if (requireProfileComplete) {
...
// Compute total payable breakdown for a bid amount on an auction
// Returns: { bid, commissionRate, commission, vatRate, vatOnBid, vatOnCommission, stcRate, stc, total }
const getPayableBreakdown = async (req, res) => {
  try {
    const { auctionId } = req.params;
    const amountRaw = req.query.amount;
    const bidAmount = Number(amountRaw);
    ...
    if (!auction) {
      auction = await Auction.findById(auctionId).select('buyersCommissionRate vatApplicable vatRate stcApplicable stcRate');
    }
    if (!auction) {
      return res.status(404).json({ success: false, message: 'Auction not found' });
    }

    // Global defaults (fallback to site settings if connected in the future)
    ...
    const vatOnBid = applyVatOnBid ? (bidAmount * vatRate) : 0;
    const stc = stcRate > 0 ? (bidAmount * stcRate) : 0;
    const total = bidAmount + commission + vatOnCommission + vatOnBid + stc;
    ...
    console.error('Get payable breakdown error:', error);
    res.status(500).json({ success: false, message: 'Failed to compute payable breakdown' });
  }
};
